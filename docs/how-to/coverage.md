...
```plain
interleavex test ./Samples/bin/net8.0/Monitors.exe -i 10 --coverage
```

This will create the directory `./Samples/bin/net8.0/Output/Monitors.exe/CoyoteOutput/`, then it
generates coverage files for code coverage which you can load into Visual Studio to see the results.

```plain
interleavex test ./Samples/bin/net8.0/Monitors.exe -i 10 -coverage activity  -o "/Coyote_Coverage/Monitors"
```
