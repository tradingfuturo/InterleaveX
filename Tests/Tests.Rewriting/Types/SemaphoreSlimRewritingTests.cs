...
{
    public class SemaphoreSlimRewritingTests : BaseRewritingTest
    {
        public SemaphoreSlimRewritingTests(ITestOutputHelper output)
            : base(output)
        {
        }
        ...
        public void TestEveryPublicSemaphoreSlimMemberHasAnExactInterceptor()
        {
            Type real = typeof(SemaphoreSlim);
            Type model = typeof(ControlledSemaphoreSlim);
            ...
            Assert.True(missing.Count is 0,
                "The controlled SemaphoreSlim model is missing exact interceptors:" + Environment.NewLine +
